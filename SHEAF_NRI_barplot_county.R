...
#
#  SHEAF_NRI_barplot_county("California", "Marin", "Brd_Total_Surface_Estimate")
...
nri_combined <-  Reduce(function(x,y) merge(x = x, y = y, by = c("State", "County", "Year", "Fips"), all = TRUE), 
  ...
  return(barplot(as.numeric(eval(parse(text=paste("nri_barplot$", NRIoption, sep="")))), names.arg = nri_barplot$Year, las = 3,  ylab = NRIoption, main = paste(NRIoption, " for ", county, " county, ", state, sep="")))
  mtext(text = c("Years"), side=1, line=2)
