SHEAF_climate_pr <- function(year) {
...
out4_count <- aggregate(out3$Fips, by=list(out3$Fips), FUN = "count")
out3a <- out3 %>% count(Fips)

colnames(out4) <- c("Fips", "Total")
out5 <- merge(counties_conus, out4, by=c("Fips"))
out5 <- merge(out5, out3a, by=c("Fips"))
out5@data$precip_average_days <- out5@data$Total / out5@data$n

pal3 <- colorNumeric(rev(brewer.pal(20, "Spectral")), na.color = "#ffffff",
                     domain = as.numeric(eval(parse(text=paste("out5$precip_average_days", sep="")))))

 label <- paste(sep = "<br/>", out5$County, as.numeric(eval(parse(text=paste("out5$precip_average_days", sep="")))))
  markers <- data.frame(label)
  labs <- as.list(as.numeric(eval(parse(text=paste("out5$precip_average_days", sep="")))))


leaflet(data = out5) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal3(as.numeric(eval(parse(text=paste("out5$precip_average_days", sep=""))))), popup = markers$label,  weight = 1)
